...
def executeScript(endpoint, authentication, user, password, ca_trust_path, ignore_validation=False, script=None, uid=None):
    ...
    try:
        ...
        if ignore_validation == "true":
            ...
            writeLog("Connecting with cert validation")
            p = winrm.protocol.Protocol(
                endpoint=endpoint,
                transport=authentication,
                username=user,
        ...
        # set codepage to utf-8 see https://docs.microsoft.com/en-us/windows/win32/intl/code-page-identifiers and https://github.com/diyan/pywinrm/issues/288
        shell = p.open_shell(env_vars=dict(WINRM_SCRIPT=encodedScript.decode("utf-8")), codepage=65001)
        writeLog("Shell opened")
        # Run command via WinRM
        command = p.run_command(shell, "powershell -EncodedCommand {script}".format(script=encodedCmd.decode("utf-8")))
        writeLog("Command executed")

        # Get reposnse
        r = winrm.Response(p.get_command_output(shell, command))
        writeLog("Response retrieved")

        p.cleanup_command(shell, command)
        p.close_shell(shell)
        writeLog("Command cleanuped and shell closed")

        writeLog("Output:\n" + r.std_out.decode("utf-8"))
        writeLog("Status code:" + str(r.status_code))

        error_decoded = r.std_err.decode("utf-8")
